...
//WARNING: Don't wear GSR device until the program has started or it will show wrong values (4095)
/*
GSR sensor development using M5StickC board. Through captive portal, we set wifi credentials in order to connect and send data to a remote server.
*/
...
void sendData(int average, char fecha[]){
  HTTPClient http;
  http.begin(serverName);
  http.addHeader("Content-Type", "application/json");

  //Montamos el json
  String cadena = "{\"sensor\": \"1\",";
  cadena += "\"value\": \"";
  cadena += String(average); 
  cadena += "\",";
  cadena += "\"date\": \"";
  cadena += fecha;
  cadena += "\"}";
...
void startSensor(){
  Serial.println("Starting reading GSR...");
  printLocalTime();
  long sum=0;
  for(int i=0;i<10;i++){
